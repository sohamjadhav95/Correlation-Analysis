...
def automate_flow(start_date_mmdd, iterations=10):
    ...
    for i in range(iterations):
        end_date_str = current_date.strftime("%m%d")
        start_date = current_date - timedelta(days=2)
        start_date_str = start_date.strftime("%m%d")
        print(f"[{i+1}/{iterations}] Processing end date: {end_date_str}, start date: {start_date_str}")

        # 2. Type Start Date (two days earlier)
        pyautogui.write(start_date_str, interval=0.05)
        time.sleep(0.5)
        ...
        # 4. Type End Date
        pyautogui.write(end_date_str, interval=0.05)
        time.sleep(0.5)
