...
const App = {
    ...
    _bindButtons() {
        ...
        // Absolute Flip tab button
        const afTabBtn = document.getElementById('btn-absolute-flip-tab');
        if (afTabBtn) {
            afTabBtn.onclick = () => {
                document.getElementById('absolute-flip-tab').classList.toggle('hidden');
            };
        }

        // Absolute Flip run button (inside the tab)
        const afRunBtn = document.getElementById('af-btn-run');
        if (afRunBtn) afRunBtn.onclick = () => AbsoluteFlipUI.run();
        ...
        themeBtn.onclick = () => {
            ...
            // Re-render Absolute Flip charts if active
            if (AppState.absoluteFlipResult && AppState.currentView === 'absolute-flip') {
                const cfg = AppState.absoluteFlipConfig;
                AbsoluteFlipUI._renderCharts(AppState.absoluteFlipResult, cfg.symbol1, cfg.symbol2);
            }
        };
    },
