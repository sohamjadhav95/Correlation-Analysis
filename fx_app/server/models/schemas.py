...
class AbsoluteFlipRequest(BaseModel):
    """Request to run distance-confirmed Absolute Flip analysis."""
    domain: Domain
    symbol_1: str
    symbol_2: str
    timeframe: Timeframe
    ...
    distance_n: float = Field(
        5.0,
        gt=0,
        description="Minimum index-point distance past zero to confirm a flip",
        examples=[5.0, 2.5, 10.0],
    )


class AbsoluteFlipResponse(BaseModel):
    """Result of an Absolute Flip analysis run."""
    status: str
    total_ticks: int
    total_bars: int
    distance_n: float
